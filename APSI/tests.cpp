...
void testFullProtocol(uint32_t numParties, uint32_t numItem, bool isEncrypted) {
    ...
    uint32_t remDepth = (uint32_t)std::ceil(std::log2(numParties));
    HE bfv("BFV", 65537, isEncrypted + remDepth, numParties);    

    std::cout << "Depth: " << isEncrypted + remDepth << std::endl;
...
void testFullProtocolTwoParty(int numParties) {
    ...
    uint32_t itemLen = 5;
    uint32_t prime = (1<<16) + 1;
    HE bfv("BFV", 65537, 0 + std::ceil(std::log2(numParties)), numParties);
