...
- ```main_apsi``` – Cong et. al. (CCS'21) implementation in OpenFHE.
...
Finally, generate the figures present in the paper, which will be placed under ```~/artifact_output``` :

```
mkdir -p ~/artifact_output
docker run --rm -v ~/artifact_output:/tmp ndss-elsa gen_charts
```

These figures are generated using the data extracted from the ```csv``` present in ```raw_data.zip```.
...
### Data Used in the Experimental Evaluation

The raw data generated by the experiments and used to produce the figures in the paper are available in the ```raw_data.zip```.

# Limitations

- The end-to-end functionality ELSA is tested on VLDP (Vehicle Loan Default Prediction) dataset only due to repository size constraints in GitHub. If you’d like to test ELSA on additional datasets, please use Amazon’s fraud dataset benchmark here: [Link](https://github.com/amazon-science/fraud-dataset-benchmark).  

- Current framework supports logistic regression only; we leave its extension to other richer functions to future work.

- KTSJ24 (PETS’24) is ommitted from the full test suite because their implementation requires OpenFHE v1.0.3 (see [dependency script](https://github.com/nd-dsp-lab/PSMT/blob/main/dependencies.sh)) and is not compatible with v1.2.3. For users who wish to test the KTSJ24's PSMT protocol, install OpenFHE v1.0.3 and execute the code under ```\ktsj24```.
